...
void sheet_slide(struct SHTCTL *ctl, struct SHEET *sht, int vx0, int vy0)
{
	...
	if (sht->height >= 0) { /* もしも表示中なら */
		sheet_refreshsub(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize); /* 新しい下じきの情報に沿って画面を描き直す */
		sheet_refreshsub(ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize); /* 新しい下じきの情報に沿って画面を描き直す */
	}
	return;
}
